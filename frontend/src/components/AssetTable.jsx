      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-gray-200">
            <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Asset Name</th>
            <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Category</th>
            <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Quantity</th>
            <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Status</th>
            <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Unit Cost</th>
            <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Actions</th>
          </tr>
        </thead>
        <tbody>
          {assets.map((asset) => (
            ...
            >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div className="text-xs text-gray-400 mt-0.5">
                        {asset.vendor && <span>{truncate(asset.vendor, 18)} · </span>}
              ...
              <td className="relative px-6 py-4">
                <div className="text-sm font-bold text-gray-900">
                  {asset.quantity}
                  {asset.unit && <span className="text-xs font-normal text-gray-400 ml-1">{asset.unit}</span>}
                </div>
              </td>

              {/* Status */}
              <td className="relative px-6 py-4">
                <span className={`inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide ${statusStyles[asset.status] ?? statusStyles['In Stock']}`}>
                  {asset.status}
                </span>
              </td>
              ...
              <td className="relative px-6 py-4">
                <div className="text-sm font-semibold text-gray-700">
                  {asset.value?.toLocaleString()} <span className="text-xs text-gray-400">{asset.currency ?? 'SAR'}</span>
                </div>
              </td>
