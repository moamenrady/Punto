...
const StatusBadge = ({ status }) => {
  ...
  return <span style={{ fontSize:'0.72rem', fontWeight:700, padding:'2px 9px', borderRadius:20, background:s.bg, color:s.color }}>{n}</span>;
};
...
const SHIFT_STYLE = {
  morning:   { bg:'#FEF3C7', color:'#D97706', dot:'#F59E0B', label:'Morning'   },
  afternoon: { bg:'#DBEAFE', color:'#2563EB', dot:'#3B82F6', label:'Afternoon' },
  night:     { bg:'#EDE9FE', color:'#7C3AED', dot:'#8B5CF6', label:'Night'     },
  off:       { bg:'#F3F4F6', color:'#9CA3AF', dot:'#D1D5DB', label:'Off'       },
  arrived:   { bg:'#D1FAE5', color:'#059669', dot:'#10B981', label:'Arrived'   },
  timeout:   { bg:'#FEE2E2', color:'#DC2626', dot:'#EF4444', label:'Timeout'   },
};
const ShiftBadge = ({ type = 'off', small = false }) => {
  const s = SHIFT_STYLE[type] ?? SHIFT_STYLE.off;
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:4, fontSize: small ? '0.68rem' : '0.75rem', fontWeight:700, padding: small ? '2px 6px' : '3px 8px', borderRadius:20, background:s.bg, color:s.color, whiteSpace:'nowrap' }}>
...
const Spinner = () => (
  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:200, gap:12 }}>
    <div style={{ width:34, height:34, border:'3px solid #E9EBF0', borderTop:'3px solid #8A9FE8', borderRadius:'50%', animation:'spin .8s linear infinite' }} />
...
const EmptyState = ({ icon = '📭', title, sub }) => (
  <div style={{ textAlign:'center', padding:'48px 16px' }}>
    ...
    {sub && <p style={{ fontSize:'0.85rem', color:'#9CA3AF', margin:0 }}>{sub}</p>}
  </div>
);
...
const ErrorBanner = ({ msg, onRetry }) => (
  <div style={{ background:'#FEF2F2', border:'1px solid #FECACA', borderRadius:10, padding:'12px 16px', marginBottom:16, display:'flex', alignItems:'center', gap:10 }}>
    <span>⚠️</span>
    <span style={{ fontSize:'0.875rem', color:'#DC2626', fontWeight:500, flex:1 }}>{msg}</span>
    {onRetry && <button onClick={onRetry} style={{ fontSize:'0.8rem', color:'#8A9FE8', background:'none', border:'none', cursor:'pointer', fontWeight:600 }}>Retry</button>}
  </div>
);

const ProjectSelector = ({ projects, value, onChange }) => (
  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
    <span style={{ fontSize:'0.78rem', fontWeight:600, color:'#6B7280', whiteSpace:'nowrap' }}>Project:</span>
    <select
      value={value}
      onChange={e => onChange(e.target.value)}
      style={{ padding:'7px 12px', borderRadius:8, border:'1px solid #E5E7EB', fontSize:'0.85rem', color:'#111827', background:'#fff', cursor:'pointer', minWidth:180 }}
    >
...
const KanbanCard = ({ task, col, onView, isDragging = false, isOverlay = false }) => {
  ...
  return (
    ...
    >
      <p style={{ fontWeight:600, fontSize:'0.875rem', color:'#111827', margin:'0 0 8px', lineHeight:1.4 }} title={task.name}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}>
        ...
        <div style={{ display:'flex', alignItems:'center' }}>
          {assignees.length === 0 ? (
            <span style={{ fontSize:'0.68rem', color:'#D1D5DB', fontStyle:'italic' }}>Unassigned</span>
          ) : (
          ...
          {assignees.length > 3 && <span style={{ fontSize:'0.68rem', color:'#9CA3AF', marginLeft:4 }}>+{assignees.length-3}</span>}
        </div>
      </div>
      {task.sprint_id?.name && (
        <p style={{ margin:'6px 0 0', fontSize:'0.7rem', color:'#8A9FE8', background:'#EEF1FD', padding:'1px 6px', borderRadius:4, display:'inline-block' }}>
          {task.sprint_id.name}
        </p>
      )}
    </div>
...
const DraggableCard = ({ task, col, onView }) => {
  ...
  return (
    ...
    >
      <KanbanCard task={task} col={col} onView={onView} isDragging={isDragging} />
    </div>
  );
};
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <span style={{ fontWeight:700, fontSize:'0.9rem', color: isOver ? '#fff' : col.accent, transition:'color .2s' }}>{col.label}</span>
        <span style={{ fontSize:'0.78rem', fontWeight:700, background: isOver ? 'rgba(255,255,255,0.3)' : col.accent, color:'#fff', borderRadius:20, padding:'1px 8px', transition:'background .2s' }}>{tasks.length}</span>
      </div>
      {/* Cards */}
      <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 8, minHeight: 120 }}>
        {tasks.length === 0 ? (
          <div style={{
            ...
            minHeight: 80, borderRadius: 8, border: `2px dashed ${isOver ? col.accent : '#E9EBF0'}`,
            transition: 'border-color .2s, background .2s',
            background: isOver ? `${col.accent}08` : 'transparent',
            padding: '16px 0',
          }}>
            <p style={{ margin:0, color: isOver ? col.accent : '#D1D5DB', fontSize:'0.82rem', fontWeight: isOver ? 600 : 400, transition:'color .2s' }}>
              {isOver ? '⬇ Drop here' : 'No tasks'}
            </p>
          </div>
        ) : (
          tasks.map(t => <DraggableCard key={t._id} task={t} col={col} onView={onView} />)
        )}
...
const Dashboard = ({ user }) => {
  ...
  const colStatusMap = { todo: 'To Do', inprogress: 'In Progress', completed: 'Completed' };
  // Maps normalized status → column key (reverse lookup)
  const statusColMap = { 'To Do': 'todo', 'In Progress': 'inprogress', 'Completed': 'completed' };

  // ─── computed ─────────────────────────────────────────────────────────────────
  const selectedProject = useMemo(() => projects.find(p => p._id === selectedProjId), [projects, selectedProjId]);
  const allTasks        = useMemo(() => Object.values(tasksByBacklog).flat(), [tasksByBacklog]);
  ...
  const stats = useMemo(() => ({
    ...
    completed:      allTasks.filter(t => normalizeStatus(t.status) === 'Completed').length,
  }), [projects, allTasks, activeSprints]);

  const kanban = useMemo(() => {
    const term = taskSearch.toLowerCase();
    const filtered = allTasks.filter(t => !term || t.name?.toLowerCase().includes(term));
    return {
      todo:       filtered.filter(t => normalizeStatus(t.status) === 'To Do'),
      inprogress: filtered.filter(t => normalizeStatus(t.status) === 'In Progress'),
      completed:  filtered.filter(t => normalizeStatus(t.status) === 'Completed'),
    };
  }, [allTasks, taskSearch]);

  const activeDragTask = useMemo(() => activeDragId ? allTasks.find(t => t._id === activeDragId) : null, [activeDragId, allTasks]);
  ...
  const teamSchedule = useMemo(() => {
    ...
    return members.map(member => {
      ...
      const done      = memberTasks.filter(t => normalizeStatus(t.status) === 'Completed').length;
      const inProg    = memberTasks.filter(t => normalizeStatus(t.status) === 'In Progress').length;
      const todo      = memberTasks.filter(t => normalizeStatus(t.status) === 'To Do').length;
  ...
  const fetchTeamSchedules = useCallback(async (pid, weekOffset) => {
    if (!pid) { setTeamSchedules([]); return; }
    try {
      setSchedLoading(true);
      const sunday    = (() => { const d = new Date(); d.setDate(d.getDate() - d.getDay() + weekOffset * 7); d.setHours(0,0,0,0); return d; })();
  ...
  const handleCreateTask = async (formData) => {
    const { backlogId, name, description, priority, sprintId, assignedTo } = formData;
    try {
  ...
  const handleUpdateTaskAssignees = async (taskId, backlogId, newIds) => {
    const updated = await projectService.updateTask(backlogId, taskId, { assigned_to: newIds });
    // Patch local state
    setTasksByBacklog(prev => ({
      ...prev,
      [backlogId]: (prev[backlogId] ?? []).map(t =>
        t._id === taskId ? { ...t, ...(updated ?? { assigned_to: newIds }) } : t
      ),
    }));
    // Keep modal data fresh
    if (modal?.data?._id === taskId) {
      setModal(prev => ({ ...prev, data: { ...prev.data, ...(updated ?? { assigned_to: newIds }) } }));
    }
  };
    ...
    setTeamSchedules(prev => {
      if (existing) {
        return prev.map(s => {
          ...
          return { ...s, entries: s.entries.map(e => e.day === day ? { ...e, shift_type: newShift } : e) };
        });
      }
      // Create a ghost schedule locally for the optimistic render
      const ghostEntries = SCHED_DAYS.map((d, i) => {
        const date = new Date(sunday); date.setDate(date.getDate() + i);
        return { day: d, date: date.toISOString(), shift_type: d === day ? newShift : 'off' };
      });
      return [...prev, { _id: `temp_${memberId}`, user_id: member, project_id: selectedProjId, week_start: weekStart, entries: ghostEntries }];
    });

    try {
      if (existing) {
        const updated = await projectService.updateScheduleEntry(existing._id, day, newShift);
        if (updated) setTeamSchedules(prev => prev.map(s => s._id === existing._id ? updated : s));
      } else {
  ...
  // ─── render helpers ───────────────────────────────────────────────────────────
  const toggleBacklog = (id) => setExpandedBacklogs(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  const toggleSprint  = (id) => setExpandedSprints (prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);

  const noProjectSelected = (
    <EmptyState icon="👆" title="Select a project" sub="Choose a project from the dropdown above to get started" />
  );

  // Compute per-project stats for the details modal
  const getProjectStats = (pid) => {
    const pSprints  = sprints.filter(s => s.project_id === pid || s.project?._id === pid);
    ...
    return {
      ...
      tasks:     pTasks.length,
      completed: pTasks.filter(t => normalizeStatus(t.status) === 'Completed').length,
    };
  };
  ...
  const renderOverview = () => (
    <>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:24 }}>
        {[
          { label:'Total Projects',   value: stats.totalProjects,  icon:'🗂️',  color:'#534AB7' },
          { label:'Total Tasks',      value: stats.totalTasks,     icon:'📋',  color:'#D97706' },
          { label:'Active Sprints',   value: stats.activeSprints,  icon:'🚀',  color:'#059669' },
          { label:'Completed Tasks',  value: stats.completed,      icon:'✅',  color:'#6B7280' },
        ].map(card => (
          <div key={card.label} className="ds-card" style={{ padding:'20px 24px' }}>
            <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between' }}>
              <div>
                <p style={{ fontSize:'0.72rem', fontWeight:700, color:'#9CA3AF', textTransform:'uppercase', letterSpacing:'0.1em', margin:'0 0 10px' }}>{card.label}</p>
                <p style={{ fontSize:'2rem', fontWeight:800, color:card.color, margin:0, lineHeight:1 }}>{card.value}</p>
              </div>
              <span style={{ fontSize:'1.5rem', opacity:0.7 }}>{card.icon}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="ds-card" style={{ padding:0, overflow:'hidden' }}>
        <div style={{ padding:'18px 24px', borderBottom:'1px solid #E9EBF0', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
          <div>
            <h2 style={{ fontSize:'1rem', fontWeight:700, color:'#111827', margin:'0 0 2px' }}>All Projects</h2>
            <p style={{ fontSize:'0.78rem', color:'#9CA3AF', margin:0 }}>{projects.length} project{projects.length !== 1 ? 's' : ''} in total</p>
          </div>
          {isAdmin && (
            <button className="ds-btn ds-btn-primary" onClick={() => setModal({ type:'createProject' })}>
              <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
        ...
        {projLoading ? <Spinner /> : projError ? <ErrorBanner msg={projError} onRetry={fetchProjects} /> : projects.length === 0 ? (
          <EmptyState icon="🗂️" title="No projects yet" sub={isAdmin ? 'Click "New Project" to get started' : 'No projects available'} />
        ) : (
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead>
                <tr style={{ borderBottom:'1px solid #E9EBF0' }}>
                  {['Project Name','Description','Team','Created At','Actions'].map(h => (
                    <th key={h} style={{ padding:'12px 20px', textAlign:'left', fontSize:'0.72rem', fontWeight:700, color:'#9CA3AF', textTransform:'uppercase', letterSpacing:'0.06em', whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {projects.map(p => (
                  <tr key={p._id} style={{ borderBottom:'1px solid #E9EBF0', transition:'background .15s' }}
                    onMouseEnter={e => e.currentTarget.style.background='#F9FAFB'}
                    onMouseLeave={e => e.currentTarget.style.background=''}
                  >
                    <td style={{ padding:'14px 20px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                        <div style={{ width:32, height:32, borderRadius:8, background:'linear-gradient(135deg,#8A9FE8,#6B82D8)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.85rem', fontWeight:800, color:'#fff', flexShrink:0 }}>
                          {p.name.charAt(0).toUpperCase()}
                        </div>
                        <button onClick={() => { setSelectedProjId(p._id); setActiveTab('backlog'); }}
                          style={{ fontWeight:700, color:'#111827', background:'none', border:'none', cursor:'pointer', fontSize:'0.9rem', textAlign:'left', padding:0, transition:'color .15s' }}
                          onMouseEnter={e => e.currentTarget.style.color='#534AB7'}
                          onMouseLeave={e => e.currentTarget.style.color='#111827'}
                        >{p.name}</button>
                      </div>
                    </td>
                    <td style={{ padding:'14px 20px', fontSize:'0.85rem', color:'#6B7280', maxWidth:200 }}>
                      <span title={p.description} style={{ display:'block', overflow:'hidden', whiteSpace:'nowrap', textOverflow:'ellipsis', maxWidth:180 }}>{p.description}</span>
                    </td>
                    <td style={{ padding:'14px 20px' }}>
                    ...
                    <td style={{ padding:'14px 20px', fontSize:'0.82rem', color:'#9CA3AF', whiteSpace:'nowrap' }}>{fmt(p.createdAt)}</td>
                    <td style={{ padding:'14px 20px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                        {/* Details */}
                        <button onClick={() => { setSelectedProjId(p._id); setModal({ type:'projectDetails', data: p }); }}
                          style={{ ...iconBtn, color:'#8A9FE8' }} title="Project details"
                          onMouseEnter={e => { e.currentTarget.style.background='#EEF1FD'; e.currentTarget.style.color='#534AB7'; }}
                          onMouseLeave={e => { e.currentTarget.style.background=''; e.currentTarget.style.color='#8A9FE8'; }}
                        >
                          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                        </button>
                        {/* Manage team */}
                        {isAdmin && (
                          <button onClick={() => { setSelectedProjId(p._id); setModal({ type:'manageTeam', data: p }); }}
                            style={{ ...iconBtn, color:'#059669' }} title="Manage team"
                            onMouseEnter={e => { e.currentTarget.style.background='#D1FAE5'; e.currentTarget.style.color='#065F46'; }}
                            onMouseLeave={e => { e.currentTarget.style.background=''; e.currentTarget.style.color='#059669'; }}
                          >
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                          </button>
                        )}
                        {/* View sprints */}
                        <button onClick={() => { setSelectedProjId(p._id); setActiveTab('sprint'); }}
                          style={{ ...iconBtn, color:'#D97706' }} title="View sprints"
                          onMouseEnter={e => { e.currentTarget.style.background='#FEF3C7'; e.currentTarget.style.color='#92400E'; }}
                          onMouseLeave={e => { e.currentTarget.style.background=''; e.currentTarget.style.color='#D97706'; }}
                        >
                        ...
                        {isAdmin && (
                          <button onClick={() => handleDeleteProject(p)}
                            style={{ ...iconBtn, color:'#EF4444' }} title="Delete project"
    ...
    <>
      ...
      {dataLoading ? <Spinner /> : dataError ? <ErrorBanner msg={dataError} onRetry={refreshProjectData} /> : backlogs.length === 0 ? (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          {backlogs.map(bl => (
            <div key={bl._id} className="ds-card" style={{ padding:0, overflow:'hidden' }}>
              {/* Backlog header */}
              <div style={{ padding:'14px 20px', background:'#F9FAFB', borderBottom:'1px solid #E9EBF0', display:'flex', alignItems:'center', gap:12, cursor:'pointer' }}
                onClick={() => toggleBacklog(bl._id)}
              >
                <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#9CA3AF" strokeWidth="2.5" style={{ transform: expandedBacklogs.includes(bl._id) ? 'rotate(90deg)' : 'rotate(0)', transition:'transform .2s', flexShrink:0 }}>
                  <polyline points="9 18 15 12 9 6"/>
                </svg>
                <div style={{ flex:1, minWidth:0 }}>
                  <span style={{ fontWeight:700, color:'#111827', fontSize:'0.9rem' }}>{bl.name}</span>
                  {bl.backlog_goal && <span style={{ fontSize:'0.78rem', color:'#9CA3AF', marginLeft:10 }}>{bl.backlog_goal}</span>}
                </div>
                <span style={{ fontSize:'0.75rem', fontWeight:600, color:'#6B7280', background:'#EEF1FD', borderRadius:20, padding:'2px 10px', flexShrink:0 }}>
                  {(tasksByBacklog[bl._id] ?? []).length} tasks
                </span>
                <div style={{ display:'flex', gap:6 }} onClick={e => e.stopPropagation()}>
                  {isAdmin && (
                    <button className="ds-btn ds-btn-primary" style={{ padding:'5px 12px', fontSize:'0.78rem' }}
                      onClick={() => setModal({ type:'createTask', data: { backlogId: bl._id } })}>
                      + Task
                    </button>
                  )}
                  {isAdmin && (
                    <button style={{ ...iconBtn, color:'#EF4444' }} title="Delete backlog"
                      onMouseEnter={e => { e.currentTarget.style.background='#FEF2F2'; }}
                      onMouseLeave={e => { e.currentTarget.style.background=''; }}
              ...
              {expandedBacklogs.includes(bl._id) && (() => {
                ...
                return tasks.length === 0 ? (
                  <div style={{ padding:'24px', textAlign:'center', color:'#9CA3AF', fontSize:'0.85rem' }}>No tasks in this backlog yet</div>
                ) : (
                  <div style={{ overflowX:'auto' }}>
                    <table style={{ width:'100%', borderCollapse:'collapse' }}>
                      <thead>
                        <tr style={{ borderBottom:'1px solid #E9EBF0' }}>
                          {['#','Task Name','Priority','Status','Sprint','Assigned To','Actions'].map(h => (
                            <th key={h} style={{ padding:'10px 16px', textAlign:'left', fontSize:'0.72rem', fontWeight:700, color:'#9CA3AF', textTransform:'uppercase', letterSpacing:'0.06em' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {tasks.map((t, i) => (
                          <tr key={t._id} style={{ borderBottom:'1px solid #F3F4F6', transition:'background .15s' }}
                            onMouseEnter={e => e.currentTarget.style.background='#F9FAFB'}
                            onMouseLeave={e => e.currentTarget.style.background=''}
                          >
                            <td style={{ padding:'10px 16px', fontSize:'0.78rem', color:'#9CA3AF', fontWeight:600 }}>{i + 1}</td>
                            <td style={{ padding:'10px 16px', fontWeight:600, fontSize:'0.875rem', color:'#111827', maxWidth:220 }}>
                              <span title={t.name} style={{ display:'block', overflow:'hidden', whiteSpace:'nowrap', textOverflow:'ellipsis', maxWidth:200 }}>{t.name}</span>
                            </td>
                            <td style={{ padding:'10px 16px' }}><PriorityBadge priority={t.priority} /></td>
                            <td style={{ padding:'10px 16px' }}><StatusBadge status={t.status} /></td>
                            <td style={{ padding:'10px 16px', fontSize:'0.82rem', color:'#6B7280' }}>
                              {t.sprint_id?.name ?? <span style={{ color:'#D1D5DB' }}>—</span>}
                            </td>
                            <td style={{ padding:'10px 16px' }}>
                              {(t.assigned_to ?? []).length === 0 ? (
                                <span style={{ fontSize:'0.75rem', color:'#D1D5DB', fontStyle:'italic' }}>Unassigned</span>
                              ) : (
                                <div style={{ display:'flex', alignItems:'center' }}>
                                  ...
                                  {(t.assigned_to??[]).length > 3 && (
                                    <span style={{ fontSize:'0.7rem', color:'#9CA3AF', marginLeft:5 }}>+{t.assigned_to.length-3}</span>
                                  )}
                                </div>
                              )}
                            </td>
                            <td style={{ padding:'10px 16px' }}>
                              <div style={{ display:'flex', gap:4 }}>
                                <button style={{ ...iconBtn }} title="View" onClick={() => setModal({ type:'viewTask', data: t })}
                                  onMouseEnter={e => { e.currentTarget.style.background='#EEF1FD'; e.currentTarget.style.color='#534AB7'; }}
                                  onMouseLeave={e => { e.currentTarget.style.background=''; e.currentTarget.style.color='#9CA3AF'; }}>
                                ...
                                {isAdmin && (
                                  <button style={{ ...iconBtn }} title="Delete" onClick={() => handleDeleteTask(t)}
                                    onMouseEnter={e => { e.currentTarget.style.background='#FEF2F2'; e.currentTarget.style.color='#EF4444'; }}
                                    onMouseLeave={e => { e.currentTarget.style.background=''; e.currentTarget.style.color='#9CA3AF'; }}>
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6M9 6V4h6v2"/></svg>
                                  </button>
  ...
  const renderSprintPlanning = () => (
    <>
      ...
      {dataLoading ? <Spinner /> : dataError ? <ErrorBanner msg={dataError} onRetry={refreshProjectData} /> : sprints.length === 0 ? (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          {sprints.map(sp => {
            const sprintTasks = allTasks.filter(t => t.sprint_id?._id === sp._id || t.sprint_id === sp._id);
            const spTodo   = sprintTasks.filter(t => normalizeStatus(t.status) === 'To Do').length;
            const spInProg = sprintTasks.filter(t => normalizeStatus(t.status) === 'In Progress').length;
            const spDone   = sprintTasks.filter(t => normalizeStatus(t.status) === 'Completed').length;
            const spTotal  = sprintTasks.length;
            const spPct    = spTotal ? Math.round((spDone / spTotal) * 100) : 0;

            return (
              <div key={sp._id} className="ds-card" style={{ padding:0, overflow:'hidden' }}>
                {/* Sprint header */}
                <div style={{ padding:'14px 20px', background:'#F9FAFB', borderBottom:'1px solid #E9EBF0', display:'flex', alignItems:'center', gap:12, cursor:'pointer' }}
                  onClick={() => toggleSprint(sp._id)}
                >
                  <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#9CA3AF" strokeWidth="2.5" style={{ transform: expandedSprints.includes(sp._id) ? 'rotate(90deg)' : 'rotate(0)', transition:'transform .2s', flexShrink:0 }}>
                    <polyline points="9 18 15 12 9 6"/>
                  </svg>
                  <div style={{ flex:1, minWidth:0, display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
                    <span style={{ fontWeight:700, color:'#111827', fontSize:'0.9rem' }}>{sp.name}</span>
                    <SprintBadge status={sp.status} />
                    {(sp.start_date || sp.end_date) && (
                      <span style={{ fontSize:'0.75rem', color:'#9CA3AF' }}>{fmt(sp.start_date)} → {fmt(sp.end_date)}</span>
                    )}
                  </div>
                  {/* Mini progress */}
                  {spTotal > 0 && (
                    <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
                      <div style={{ display:'flex', gap:4 }}>
                        <span style={{ fontSize:'0.7rem', color:'#534AB7', fontWeight:700, background:'#EEF1FD', padding:'1px 6px', borderRadius:4 }}>{spTodo} Todo</span>
                        <span style={{ fontSize:'0.7rem', color:'#92400E', fontWeight:700, background:'#FEF3C7', padding:'1px 6px', borderRadius:4 }}>{spInProg} In Progress</span>
                        <span style={{ fontSize:'0.7rem', color:'#065F46', fontWeight:700, background:'#D1FAE5', padding:'1px 6px', borderRadius:4 }}>{spDone} Done</span>
                      </div>
                      <div style={{ width:60, height:6, background:'#E9EBF0', borderRadius:99, overflow:'hidden' }}>
                        <div style={{ height:'100%', width:`${spPct}%`, background:'linear-gradient(90deg,#534AB7,#3ECFAA)', borderRadius:99 }} />
                      </div>
                      <span style={{ fontSize:'0.7rem', fontWeight:700, color:'#059669' }}>{spPct}%</span>
                    </div>
                  )}
                  <div style={{ display:'flex', gap:6 }} onClick={e => e.stopPropagation()}>
                    {isAdmin && (
                      <button className="ds-btn ds-btn-primary" style={{ padding:'5px 12px', fontSize:'0.78rem' }}
                        onClick={() => setModal({ type:'createTask', data: { sprintId: sp._id, backlogId: backlogs[0]?._id ?? '' } })}>
                        + Task
                      </button>
                    )}
                    {isAdmin && (
                      <button className="ds-btn ds-btn-secondary" style={{ padding:'5px 12px', fontSize:'0.78rem' }}
                        onClick={() => setModal({ type:'editSprint', data: sp })}>
                        Edit
                      </button>
                    )}
                    {isAdmin && (
                      <button style={{ ...iconBtn, color:'#EF4444' }} title="Delete sprint"
                        onMouseEnter={e => { e.currentTarget.style.background='#FEF2F2'; }}
                        onMouseLeave={e => { e.currentTarget.style.background=''; }}
                        onClick={() => setModal({ type:'deleteSprint', data: sp })}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>
                      </button>
                    )}
                  </div>
                </div>
                ...
                {expandedSprints.includes(sp._id) && (
                  sprintTasks.length === 0 ? (
                    <div style={{ padding:'20px', textAlign:'center', color:'#9CA3AF', fontSize:'0.85rem' }}>No tasks assigned to this sprint</div>
                  ) : (
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, padding:16 }}>
                      {[
                        { label:'Todo',        tasks: sprintTasks.filter(t => normalizeStatus(t.status) === 'To Do'),       accent:'#534AB7', bg:'#EEF1FD' },
                        { label:'In Progress', tasks: sprintTasks.filter(t => normalizeStatus(t.status) === 'In Progress'),  accent:'#D97706', bg:'#FEF9C3' },
                        { label:'Done',        tasks: sprintTasks.filter(t => normalizeStatus(t.status) === 'Completed'),    accent:'#059669', bg:'#D1FAE5' },
                      ].map(col => (
                        <div key={col.label} style={{ background:'#FAFAFA', border:'1px solid #E9EBF0', borderRadius:10, overflow:'hidden' }}>
                          <div style={{ padding:'8px 12px', background:col.bg, borderBottom:`1px solid ${col.accent}20`, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                            <span style={{ fontSize:'0.78rem', fontWeight:700, color:col.accent }}>{col.label}</span>
                            <span style={{ fontSize:'0.7rem', fontWeight:700, color:col.accent, background:`${col.accent}20`, borderRadius:20, padding:'1px 7px' }}>{col.tasks.length}</span>
                          </div>
                          <div style={{ padding:8, display:'flex', flexDirection:'column', gap:6, minHeight:60 }}>
                            {col.tasks.length === 0 ? (
                              <p style={{ textAlign:'center', fontSize:'0.75rem', color:'#D1D5DB', padding:'8px 0', margin:0 }}>Empty</p>
                            ) : col.tasks.map(t => (
                              <div key={t._id}
                                style={{ background:'#fff', border:'1px solid #E9EBF0', borderRadius:7, padding:'8px 10px', cursor:'pointer', transition:'box-shadow .15s' }}
                                onClick={() => setModal({ type:'viewTask', data: t })}
                                onMouseEnter={e => e.currentTarget.style.boxShadow='0 2px 6px rgba(0,0,0,0.07)'}
                                onMouseLeave={e => e.currentTarget.style.boxShadow=''}
                              >
                                <p style={{ margin:'0 0 4px', fontSize:'0.8rem', fontWeight:600, color:'#111827', overflow:'hidden', whiteSpace:'nowrap', textOverflow:'ellipsis' }}
  ...
  const renderBoard = () => {
    const colCfg = [
      ...
      { key:'completed',  label:'Completed',    accent:'#059669', bg:'#D1FAE5' },
    ];
              ...
              <DragOverlay dropAnimation={{ duration: 180, easing: 'cubic-bezier(0.18,0.67,0.6,1.22)' }}>
                {activeDragTask ? (
                  <div style={{ width: 260 }}>
                    <KanbanCard
                      task={activeDragTask}
                      col={colCfg.find(c => c.key === (statusColMap[normalizeStatus(activeDragTask.status)] ?? 'todo')) ?? colCfg[0]}
                      onView={() => {}}
                      isOverlay={true}
                    />
                  </div>
  ...
  const renderTeamSchedule = () => {
    ...
    teamSchedules.forEach(s => {
      ...
      schedMap[uid] = {};
      (s.entries ?? []).forEach(e => { schedMap[uid][e.day] = e.shift_type; });
    });

    const getShift = (memberId, day) => schedMap[String(memberId)]?.[day] ?? 'off';
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
            ...
            <div className="team-stats-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
              {[
                ...
                { label:'Completed',   value: allTasks.filter(t => normalizeStatus(t.status)==='Completed').length,      color:'#059669', bg:'#D1FAE5', icon:'✅' },
              ].map(s => (
            ...
            <div className="ds-card" style={{ padding:0, overflow:'hidden' }}>
              ...
              <div style={{ padding:'14px 20px', borderBottom:'1px solid #E9EBF0', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10, background:'#FAFAFA' }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#534AB7" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                  <span style={{ fontWeight:700, fontSize:'0.95rem', color:'#111827' }}>Weekly Shift Schedule</span>
                  {isAdmin && <span style={{ fontSize:'0.72rem', color:'#9CA3AF', fontWeight:500 }}>— click a cell to change shift</span>}
                </div>
                {/* Week navigator */}
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  <button onClick={() => setSchedWeekOffset(p => p - 1)} title="Previous week"
                    style={{ ...iconBtn, border:'1px solid #E9EBF0', borderRadius:8, width:30, height:30 }}
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 14px', background:'#EEF1FD', borderRadius:8, border:'1px solid #C7D2F8', minWidth:180, justifyContent:'center' }}>
                    <span style={{ fontSize:'0.82rem', fontWeight:700, color:'#534AB7' }}>{weekLabel}</span>
                    {isThisWeek && <span style={{ fontSize:'0.65rem', fontWeight:700, background:'#534AB7', color:'#fff', borderRadius:20, padding:'1px 7px' }}>This Week</span>}
                  </div>
                  <button onClick={() => setSchedWeekOffset(p => p + 1)} title="Next week"
                    style={{ ...iconBtn, border:'1px solid #E9EBF0', borderRadius:8, width:30, height:30 }}
                    onMouseEnter={e => e.currentTarget.style.background='#EEF1FD'}
                    onMouseLeave={e => e.currentTarget.style.background='transparent'}
                  >
                  ...
                  {schedWeekOffset !== 0 && (
                    <button onClick={() => setSchedWeekOffset(0)}
                      style={{ fontSize:'0.75rem', fontWeight:700, color:'#8A9FE8', background:'none', border:'1px solid #C7D2F8', borderRadius:8, padding:'4px 10px', cursor:'pointer' }}>
                      Today
                    </button>
                  )}
                </div>
              ...
              {/* Legend */}
              <div style={{ padding:'8px 20px', borderBottom:'1px solid #E9EBF0', display:'flex', alignItems:'center', gap:10, flexWrap:'wrap', background:'#fff' }}>
                <span style={{ fontSize:'0.7rem', fontWeight:700, color:'#9CA3AF', textTransform:'uppercase', letterSpacing:'0.08em', marginRight:4 }}>Shifts:</span>
              ...
              {schedLoading ? <Spinner /> : (
                <div style={{ overflowX:'auto' }}>
                  <table style={{ width:'100%', borderCollapse:'collapse', minWidth:640 }}>
                    <thead>
                      <tr style={{ background:'#F9FAFB', borderBottom:'1px solid #E9EBF0' }}>
                        <th style={{ padding:'10px 16px', textAlign:'left', fontSize:'0.72rem', fontWeight:700, color:'#9CA3AF', textTransform:'uppercase', letterSpacing:'0.06em', whiteSpace:'nowrap', width:180 }}>Member</th>
                        {SCHED_DAYS.map((d, i) => {
                          const date = new Date(sunday); date.setDate(date.getDate() + i);
                          const isToday = date.toDateString() === new Date().toDateString();
                          return (
                            <th key={d} style={{ padding:'8px 6px', textAlign:'center', fontSize:'0.72rem', fontWeight:700, color: isToday ? '#534AB7' : '#9CA3AF', textTransform:'uppercase', letterSpacing:'0.06em', minWidth:90, background: isToday ? '#EEF1FD' : undefined, borderBottom: isToday ? '2px solid #534AB7' : undefined }}>
                              <div>{d}</div>
                              <div style={{ fontSize:'0.68rem', fontWeight:500, color: isToday ? '#8A9FE8' : '#C9D1D9' }}>{date.toLocaleDateString('en-US', { month:'short', day:'numeric' })}</div>
                            </th>
                          );
                        })}
                      </tr>
                    ...
                    <tbody>
                      {members.map((member, ri) => (
                        <tr key={member._id} style={{ borderBottom:'1px solid #F3F4F6', background: ri % 2 === 0 ? '#fff' : '#FAFAFA' }}>
                          {/* Member info cell */}
                            ...
                            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                              ...
                              <div style={{ minWidth:0 }}>
                                ...
                                <span style={{ fontSize:'0.65rem', fontWeight:700, padding:'1px 5px', borderRadius:20,
                                  background: member.role === 'admin' ? '#FEE2E2' : member.role === 'manager' ? '#FEF3C7' : '#EEF1FD',
                                  color:      member.role === 'admin' ? '#DC2626' : member.role === 'manager' ? '#D97706' : '#534AB7',
                          ...
                          {SCHED_DAYS.map((day, di) => {
                            const date    = new Date(sunday); date.setDate(date.getDate() + di);
                            const isToday = date.toDateString() === new Date().toDateString();
                            const shift   = getShift(member._id, day);
                            const ss      = SHIFT_STYLE[shift] ?? SHIFT_STYLE.off;
                            return (
                              <td key={day} style={{ padding:'6px', textAlign:'center', background: isToday ? '#F5F6FF' : undefined }}>
                                {isAdmin ? (
                                  <select
                                    value={shift}
                                    onChange={e => handleShiftChange(member, day, e.target.value)}
                                    style={{
                                  ...
                                  >
                                    {Object.keys(SHIFT_STYLE).map(t => (
                                      <option key={t} value={t}>{SHIFT_STYLE[t].label}</option>
                                    ))}
                                  </select>
                                ) : (
            ...
            <div>
              <h3 style={{ fontSize:'0.85rem', fontWeight:700, color:'#9CA3AF', textTransform:'uppercase', letterSpacing:'0.08em', margin:'0 0 12px' }}>Task Progress</h3>
                  ...
                  <div key={member._id} className="ds-card" style={{ padding:'18px 22px' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
                      ...
                      <div style={{ textAlign:'right' }}>
                        <p style={{ margin:0, fontSize:'1.3rem', fontWeight:800, color: pct===100?'#059669':pct>=50?'#D97706':'#534AB7' }}>{pct}%</p>
                        <p style={{ margin:0, fontSize:'0.68rem', color:'#9CA3AF', fontWeight:600 }}>done</p>
                      </div>
                    </div>
                    <div style={{ marginBottom:12 }}>
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                        <span style={{ fontSize:'0.72rem', fontWeight:600, color:'#6B7280' }}>Progress</span>
                        <span style={{ fontSize:'0.72rem', fontWeight:700, color:'#374151' }}>{done}/{total} tasks</span>
                      </div>
                      <div style={{ height:7, background:'#E9EBF0', borderRadius:99, overflow:'hidden' }}>
                        <div style={{ height:'100%', width:`${pct}%`, background:pct===100?'linear-gradient(90deg,#059669,#3ECFAA)':'linear-gradient(90deg,#534AB7,#8A9FE8)', borderRadius:99, transition:'width .6s ease' }} />
                      </div>
                    </div>
                    <div style={{ display:'flex', gap:6 }}>
                      {[{v:todo,l:'To Do',c:'#534AB7',bg:'#EEF1FD'},{v:inProg,l:'In Prog',c:'#D97706',bg:'#FEF3C7'},{v:done,l:'Done',c:'#059669',bg:'#D1FAE5'}].map(s=>(
                        <div key={s.l} style={{ flex:1, background:s.bg, borderRadius:8, padding:'8px', textAlign:'center' }}>
                          <p style={{ margin:0, fontWeight:800, fontSize:'1.1rem', color:s.c }}>{s.v}</p>
                          <p style={{ margin:0, fontSize:'0.65rem', fontWeight:700, color:s.c, textTransform:'uppercase', opacity:0.75 }}>{s.l}</p>
                        </div>
                      ))}
                    </div>
                    <div style={{ marginTop:10, paddingTop:10, borderTop:'1px solid #F3F4F6', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                      <span style={{ fontSize:'0.7rem', fontWeight:700, padding:'2px 7px', borderRadius:20,
                        background:member.role==='admin'?'#FEE2E2':member.role==='manager'?'#FEF3C7':'#EEF1FD',
                        color:member.role==='admin'?'#DC2626':member.role==='manager'?'#D97706':'#534AB7',
                      }}>{member.role??'user'}</span>
                      {total===0 && <span style={{ fontSize:'0.73rem', color:'#D1D5DB' }}>No tasks assigned</span>}
                      {total>0 && pct===100 && <span style={{ fontSize:'0.73rem', color:'#059669', fontWeight:700 }}>🎉 All done!</span>}
                      {total>0 && pct<100 && <span style={{ fontSize:'0.73rem', color:'#9CA3AF' }}>{total-done} remaining</span>}
                    </div>
                  </div>
                ))}
              </div>
  ...
  const sprintForEdit = modal?.data ? {
    ...modal.data,
    startDate: modal.data.start_date ? modal.data.start_date.slice(0, 10) : '',
