...
const AddMemberModal = ({ isOpen, onClose, project, onMembersChange }) => {
    ...
    const memberIds = new Set((project.members ?? []).map(m => m._id ?? m));
    ...
    const handleAdd = async (user) => {
        ...
        try {
            ...
            setActionMap(prev => { const n = { ...prev }; delete n[user._id]; return n; });
        }
    };
    ...
    const UserRow = ({ user, isMember }) => {
        ...
        return (
            <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 0', borderBottom:'1px solid #F3F4F6' }}>
                <div style={{ width:36, height:36, borderRadius:'50%', background:'linear-gradient(135deg,#8A9FE8,#6B82D8)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.82rem', fontWeight:800, color:'#fff', flexShrink:0 }}>
                    {user.name?.split(' ').map(n => n[0]).slice(0,2).join('').toUpperCase()}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                    <p style={{ margin:0, fontWeight:700, fontSize:'0.875rem', color:'#111827', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{user.name}</p>
                    <p style={{ margin:0, fontSize:'0.75rem', color:'#9CA3AF', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{user.email}</p>
                </div>
                <span style={{ fontSize:'0.68rem', fontWeight:700, padding:'2px 8px', borderRadius:20, background:roleBg(user.role), color:roleColor(user.role), flexShrink:0 }}>
                    {user.role ?? 'user'}
                </span>
                {isMember ? (
                    <button
                        ...
                        style={{ padding:'5px 12px', fontSize:'0.78rem', fontWeight:600, borderRadius:6, border:'1px solid #FECACA', background:'#FEF2F2', color:'#DC2626', cursor:busy?'wait':'pointer', flexShrink:0, opacity:busy?0.6:1 }}>
                        {busy ? '…' : 'Remove'}
                    </button>
                ) : (
                    <button
                        ...
                        style={{ padding:'5px 12px', fontSize:'0.78rem', fontWeight:600, borderRadius:6, border:'1px solid #C7D2F8', background:'#EEF1FD', color:'#534AB7', cursor:busy?'wait':'pointer', flexShrink:0, opacity:busy?0.6:1 }}>
                        {busy ? '…' : '+ Add'}
                    </button>
                )}
            </div>
            ...
            <div className="ds-modal" style={{ maxWidth:520, width:'100%' }}>
                ...
                <div className="ds-modal-header">
                    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                        ...
                        <div>
                            <h2 className="ds-modal-title" style={{ marginBottom:0 }}>Manage Team</h2>
                            <p style={{ margin:0, fontSize:'0.75rem', color:'#9CA3AF' }}>{project.name}</p>
                        </div>
                    </div>
                ...
                <div className="ds-modal-body" style={{ display:'flex', flexDirection:'column', gap:16 }}>
                    ...
                    <div>
                        ...
                        {currentMembers.length === 0 ? (
                            <p style={{ textAlign:'center', color:'#D1D5DB', fontSize:'0.85rem', padding:'16px 0' }}>No members yet — search above to add</p>
                        ) : (
                ...
                <div className="ds-modal-footer">
                    <button type="button" className="ds-btn ds-btn-secondary" onClick={onClose}>Done</button>
                </div>
            </div>
        </div>
    );
