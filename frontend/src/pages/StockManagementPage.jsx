...
const StockManagementPage = ({ currentUserRole }) => {
  ...
  const fetchAssets = useCallback(async () => {
    try {
      ...
      if (!res.ok) throw new Error(`Error ${res.status}: Failed to load assets`);
      const json = await res.json();
      const items = Array.isArray(json) ? json : (json.data?.data ?? json.data ?? []);
  ...
  const handleReduce = async (id, quantityToReduce, reason) => {
    ...
    try {
      ...
      setAssets((prev) =>
        prev.map((a) =>
          a.id === id
            ? { ...a, quantity: newQty, status: computeStatus(newQty, a.minimumThreshold) }
            : a
        )
      );
      ...
      <div className="page-content-wrapper">
        ...
        {isLoading ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 200, gap: 12 }}>
            <div style={{ width: 36, height: 36, border: '3px solid #E9EBF0', borderTop: '3px solid #8A9FE8', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
            <p style={{ color: '#9CA3AF', fontSize: '0.875rem' }}>Loading assets...</p>
          </div>
        ) : (
          <>
            {/* Summary Stats */}
                ...
                <div key={stat.label} className="ds-card" style={{ padding: '20px 24px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    <div>
                      ...
                      <p style={{ fontSize: stat.label === 'Total Value' ? '1.3rem' : '2rem', fontWeight: 800, color: stat.valueColor, margin: 0, lineHeight: 1 }}>{stat.value}</p>
                    </div>
