...
const handleLogin = async (e) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-4 transition-all duration-500 ${theme.bg}`}>
      <button
      ...
      >
        {isDarkMode ? <Sun size={18} className="text-[#E2E0FF]" /> : <Moon size={18} className="text-[#534AB7]" />}
      </button>
          ...
          {view === "login" ? (
            <motion.div key="login" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              <div className="text-center mb-8">
                ...
                <h2 className="text-[24px] font-bold mb-1" style={{ color: theme.primary }}>Welcome back!</h2>
                <p className={`text-[13px] ${theme.textM}`}>
                  Don't have an account?
                  <Link to="/signup" className="font-bold hover:underline ml-1" style={{ color: theme.primary }}>Sign up</Link>
                </p>
              </div>

              <div className="space-y-3 mb-8">
                <motion.button whileHover={socialHover} whileTap={{ scale: 0.98 }} className={`w-full flex items-center justify-center gap-2.5 py-3 border rounded-xl transition-all ${theme.border} ${theme.textP} ${theme.input}`}>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} placeholder="Password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })}
                    ...
                    style={{ "--tw-ring-color": theme.primary }} />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className={`absolute right-4 top-3.5 ${theme.textM}`}>
              ...
              <div className="mt-8 text-center">
                <button onClick={() => { setView("forgot"); setApiError(""); setApiSuccess(""); }} className="text-[13px] font-medium hover:underline" style={{ color: theme.primary }}>Forgot Password?</button>
              </div>
            </motion.div>
          ) : (
            <motion.div key="forgot" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              <div className="text-center mb-12">
                {isDarkMode ? <DarkLogo primary={theme.primary} accent={theme.accent} /> : <LightLogo />}
                <h2 className="text-[32px] font-bold mb-2 mt-4" style={{ color: isDarkMode ? "#E2E0FF" : "#1E1B3A" }}>Forgot <span className={isDarkMode ? "bg-gradient-to-r from-[#7F6FF5] to-[#3ECFAA] bg-clip-text text-transparent" : ""} style={{ color: !isDarkMode ? "#534AB7" : "transparent" }}>Password?</span></h2>
                <p className={`text-[14px] ${theme.textM}`}>Enter your email to reset your account</p>
              </div>
              <form onSubmit={handleForgot} className="space-y-8 mt-16">
                <div className="relative">
                  <input type="email" placeholder="Enter work email" required value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                    className={`w-full px-5 py-4 rounded-xl border outline-none text-[14px] ${theme.input} ${theme.textP} ${theme.border} focus:ring-1`}
                    style={{ "--tw-ring-color": theme.primary }} />
                  <Mail className={`absolute right-4 top-4.5 ${theme.textM}`} size={20} />
                </div>
                {apiError && <div className="bg-red-50 text-red-500 text-[12px] p-3 rounded-xl text-center border border-red-100">{apiError}</div>}
                {apiSuccess && <div className="bg-green-50 text-green-600 text-[12px] p-4 rounded-xl text-center font-bold border border-green-100 shadow-sm">{apiSuccess}</div>}
                <button type="submit" disabled={isLoading} className={`w-full py-4 rounded-xl text-white font-bold text-[16px] bg-gradient-to-r shadow-lg transition-all hover:scale-[1.01] ${theme.btn}`}>
                  {isLoading ? <Loader2 className="animate-spin mx-auto" size={24} /> : "Send Reset Link"}
                </button>
                <div className="text-center mt-4">
                  <button type="button" onClick={() => setView("login")} className="text-sm font-bold opacity-70 hover:opacity-100" style={{ color: theme.primary }}>Back to Login</button>
                </div>
              </form>
            </motion.div>
          )}
