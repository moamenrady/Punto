...
const Table = require('./models/stockModel');
...
const seed = async () => {
    try {
        ...
        const plans = await Plan.find({ name: { $in: ['Basic', 'Pro', 'Enterprise'] } });
        const techFlow = await Company.findOne({ name: 'TechFlow Solutions v2' });
        const users = await User.find({ company_id: techFlow?._id });
        const project = await Project.findOne({ name: 'Punto Platform Refactor v2' });
        const sprint = await Sprint.findOne({ name: 'Sprint 1: Infrastructure v2' });
        const backlog = await Backlog.findOne({ name: 'Core Features v2' });

        if (!techFlow || users.length < 3) {
            console.error('Basic v2 data not found. Please run the initial seeder first.');
            process.exit(1);
        }
        ...
        if (!stockExists) {
            await Table.create({
                ...
                data: [
                    new Map([['Item', 'MacBook Pro'], ['Quantity', '10'], ['Status', 'In Stock']]),
                    new Map([['Item', 'Dell Monitor'], ['Quantity', '25'], ['Status', 'Low Stock']]),
                    new Map([['Item', 'Logitech Mouse'], ['Quantity', '50'], ['Status', 'In Stock']])
                ]
            });
        ...
        if (!scheduleExists) {
            ...
            await Schedule.create({
                ...
                entries: [
                    ...
                    { day: 'Mon', date: new Date(startOfWeek.getTime() + 86400000), shift_type: 'morning' },
                    { day: 'Tue', date: new Date(startOfWeek.getTime() + 172800000), shift_type: 'night' },
                    { day: 'Wed', date: new Date(startOfWeek.getTime() + 259200000), shift_type: 'off' }
                ]
            });
        ...
        const notifExists = await Notification.findOne({ company_id: techFlow._id });
        if (!notifExists) {
        ...
        if (!paymentExists) {
            await Payment.create({
                user_id: users[0]._id,
                amount: 149,
                features: ['Project Management', 'Chat System', 'Ticketing System', 'Stock Management'],
                paymentMethod: 'visa',
                status: 'paid',
        ...
        const chatExists = await Chat.findOne({ company_id: techFlow._id, type: 'group' });
        if (!chatExists) {
